...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Login administrativo com autenticação server-side e sessão HttpOnly.",
      },
...
function LoginPage() {
      ...
      .catch(() => {
        if (active) {
          setStatus({
            ...
            message: "Não foi possível consultar o serviço de autenticação.",
          });
        }
      })
      .finally(() => {
  ...
  const handleLoginSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const authenticatedUser = await refreshAuthSession();
      if (!authenticatedUser) {
        setErrorMsg("A sessão foi criada, mas não pôde ser validada pelo servidor.");
        return;
      }

      setSuccessUser(payload.user);
      const redirect = getSafeRedirect(searchParams.redirect);
      window.setTimeout(() => window.location.replace(redirect), 350);
    } catch {
  ...
  const authUnavailable = Boolean(
    checkingStatus || !status || status.lockdownEnabled || !status.configured,
  );
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden bg-background">
      ...
      <main className="w-full max-w-md glass-card p-6 md:p-8 relative shadow-2xl border border-border/60">
        ...
        {searchParams.signedOut === "1" && !successUser ? (
          <div
            className="mb-4 rounded-lg border border-primary/25 bg-primary/10 p-3 text-xs text-foreground"
            role="status"
          >
        ...
        {successUser ? (
          <div className="py-8 text-center flex flex-col items-center gap-3" role="status">
          ...
          <form onSubmit={handleLoginSubmit} noValidate className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative flex items-center">
                <User
                  className="h-4 w-4 absolute left-3 text-muted-foreground pointer-events-none"
                  aria-hidden="true"
                />
                <input
                  id={loginInputId}
                  ...
                  aria-describedby={[statusId, errorMsg ? errorId : undefined]
                    .filter(Boolean)
                    .join(" ")}
                  className="h-11 w-full pl-9 pr-3 rounded-lg border border-input bg-background/60 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all disabled:opacity-60"
                />
              </div>
            </div>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  aria-describedby={[statusId, errorMsg ? errorId : undefined]
                    .filter(Boolean)
                    .join(" ")}
                  className="h-11 w-full pl-9 pr-10 rounded-lg border border-input bg-background/60 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all disabled:opacity-60"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((visible) => !visible)}
                  disabled={authUnavailable || loading}
                  className="absolute right-3 text-muted-foreground hover:text-foreground p-1 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded disabled:opacity-60"
            ...
            {errorMsg ? (
              <div
                ...
                role="alert"
                aria-live="polite"
                className="flex items-start gap-2.5 text-xs text-destructive bg-destructive/10 border border-destructive/30 rounded-lg p-3"
              >
