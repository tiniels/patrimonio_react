...
const ADMIN_ROLES = ["admin", "contabilidade", "chefia", "galpao"] as const satisfies readonly UserRole[];
...
export async function handleAuthApiRequest(request: Request, env: unknown): Promise<Response | null> {
  ...
  if (url.pathname === "/api/v1/auth/status" && request.method === "GET") {
    return jsonResponse(getPublicStatus(env as RuntimeEnv), { correlationId });
  }
  ...
  if (url.pathname === "/api/v1/auth/admin/login") {
    ...
    return handleAdminLogin(request, env as RuntimeEnv, correlationId);
  }

  if (url.pathname === "/api/v1/auth/session" && request.method === "GET") {
    return handleGetSession(request, env as RuntimeEnv, correlationId);
  }

  if (url.pathname === "/api/v1/auth/session" && (request.method === "DELETE" || request.method === "POST")) {
    return handleLogout(correlationId);
  }
...
async function handleAdminLogin(
  ...
  if (!login || !password || login.length > MAX_LOGIN_LENGTH || password.length > MAX_PASSWORD_LENGTH) {
    ...
    return problemResponse(400, "validation_failed", "Informe usuário e senha em formato válido.", correlationId);
  }
  ...
  if (!user) {
    ...
    return problemResponse(401, "invalid_credentials", "Usuário ou senha inválidos.", correlationId);
  }
...
async function handleGetSession(request: Request, env: RuntimeEnv, correlationId: string): Promise<Response> {
  const sessionCookie = parseCookies(request.headers.get("cookie") ?? "")[SESSION_COOKIE_NAME];

  if (!sessionCookie) {
    return jsonResponse({ authenticated: false, user: null }, { status: 200, correlationId });
  }
...
function handleLogout(correlationId: string): Response {
...
function getPublicStatus(env: RuntimeEnv): AuthStatus {
  ...
  return {
    ...
    message: getStatusMessage(lockdownEnabled, configured, provider, sessionSecret, passwordHash),
  };
}
...
function getStatusMessage(
  ...
  if (lockdownEnabled) {
    return "A autenticação administrativa permanece bloqueada por AUTH_LOCKDOWN_ENABLED. Desative somente após configurar o provedor e validar segurança.";
  }
  if (configured) {
    return "Autenticação administrativa server-side configurada.";
  }
  if (provider !== "local-admin") {
    return "AUTH_PROVIDER deve ser local-admin até a integração OIDC institucional ser homologada.";
  }
  if (sessionSecret.length < 32) {
    return "SESSION_SECRET precisa existir no ambiente seguro e possuir pelo menos 32 caracteres.";
  }
  if (!isSupportedPasswordHash(passwordHash)) {
    return "ADMIN_BOOTSTRAP_PASSWORD_HASH deve usar o formato pbkdf2_sha256$iteracoes$saltBase64Url$hashBase64Url.";
  }
  if (!isWebCryptoAvailable()) {
    return "WebCrypto indisponível no runtime server-side.";
  }
  return "Configuração de autenticação incompleta.";
}

async function verifyLocalAdmin(env: RuntimeEnv, login: string, password: string): Promise<AuthUser | null> {
...
async function createSessionCookie(user: AuthUser, env: RuntimeEnv): Promise<string> {
...
function checkRateLimit(
  ...
  const now = Date.now();
  const windowMs = getNumberEnv(env, "AUTH_RATE_LIMIT_WINDOW_SECONDS", DEFAULT_RATE_LIMIT_WINDOW_SECONDS) * 1000;
  const maxAttempts = getNumberEnv(env, "AUTH_RATE_LIMIT_MAX_ATTEMPTS", DEFAULT_RATE_LIMIT_MAX_ATTEMPTS);
...
function recordFailedAttempt(request: Request, login: string, env: RuntimeEnv): void {
  ...
  current.count += 1;
  ATTEMPT_BUCKETS.set(key, current);
}
...
async function readJsonBody(
  ...
  if (!contentType.includes("application/json")) {
    return {
      ok: false,
      response: problemResponse(415, "unsupported_media_type", "Envie a requisição como JSON.", correlationId),
    };
  }
...
function jsonResponse(
  data: unknown,
  options: { status?: number; correlationId?: string; headers?: Record<string, string> } = {},
): Response {
...
function parseAdminRole(value: string | UserRole): AdminRole {
...
function getCorrelationId(request: Request): string {
  return request.headers.get("x-correlation-id") || `req_${randomToken(12)}`;
}
